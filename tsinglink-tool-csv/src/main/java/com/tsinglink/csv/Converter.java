package com.tsinglink.csv;

import com.tsinglink.csv.entity.CsvProperty;
...
public class Converter {
	 ...
	 * 标题名-标题index
	 */
	private final Map<String, Integer> titleIndexMap = new LinkedHashMap<>();
	 ...
	 * @return
	 */
	public <T> T getT(String str, Class<T> classA)
	 ...
	 * @return
	 */
	private <T> T getT(String[] cells, Class<T> a, Map<String, String> titleFieldMap)
			throws InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	 ...
	 */
	public <T> void invoke(String cellValue, Method method, T result)
	...
	public <T> void invoke(String cellValue, Field field, T result)
...
//        String parameterTypeStr = parameterType.toString();
		...
		try {
			if (parameterType.equals(BigDecimal.class)) {
				...
				result = cellValue;
			} else if (parameterType.equals(Integer.class) || "int".equals(
					parameterType.getName())) {
				result = parseInt(cellValue);
			} else if (parameterType.equals(Float.class) || "float".equals(
					parameterType.getName())) {
				result = parseFloat(cellValue);
			} else if (parameterType.equals(Double.class) || "double".equals(
					parameterType.getName())) {
			...
			} else if (parameterType.equals(Short.class) || "short".equals(
					parameterType.getName())) {
				result = parseShort(cellValue);
			} else if (parameterType.equals(Boolean.class) || "boolean".equals(
					parameterType.getName())) {
				result = Boolean.parseBoolean(cellValue);
			} else if (parameterType.equals(Character.class) || "char".equals(
					parameterType.getName())) {
				result = cellValue.charAt(0);
			}
		} catch (Exception e) {
			System.out.println(
					"类型转换异常-无法将值‘" + cellValue + "’转换成‘" + parameterType + "’类型");
			e.printStackTrace();
		}
		return result;
	}
